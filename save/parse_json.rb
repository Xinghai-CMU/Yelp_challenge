...
CSV.open('business.csv', "w") do |csv0|
	...
	while ((s=f.gets)!=nil)
		...
		csv0<<[business_num[parsed_json["business_id"]],parsed_json["open"],parsed_json["categories"],parsed_json["state"],parsed_json["city"],parsed_json["longitude"],parsed_json["latitude"],parsed_json["stars"],parsed_json["review_count"]]	
	end	
end
...
CSV.open('review.csv', "w") do |csv2|
	...
	while ((s=f.gets)!=nil)
		...
		cur_arr = business_with_label[business_num[parsed_json["business_id"]]]
...
CSV.open('user_business.csv', "w") do |csv4|
